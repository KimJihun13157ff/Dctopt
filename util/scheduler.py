...
class Warmup_LamdbaLR(LambdaLR):
    def __init__(self, optimizer, warmup_step=10, gamma=0.998, init_rate=0.0, last_epoch=-1):
        self.optimizer = optimizer
        self.warmup_step = warmup_step
        self.gamma = gamma
        self.init_rate = init_rate
        self.last_epoch = last_epoch
        super(Warmup_LamdbaLR, self).__init__(optimizer, self.lr_lambda, last_epoch=last_epoch)
    ...
    def lr_decay(self, step):
        lr_rate = self.gamma ** (step - self.warmup_step)
        return lr_rate
